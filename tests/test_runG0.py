...
def test_run_g0_flow(cli_cmd):
    ...
    din_coord_artifact = HARDHAT_ARTIFACTS / Path("DinCoordinator.sol/DinCoordinator.json")
    ...
    din_token_artifact = HARDHAT_ARTIFACTS / Path("DinToken.sol/DinToken.json")
    ...
    din_stake_artifact = HARDHAT_ARTIFACTS / Path("DinValidatorStake.sol/DinValidatorStake.json")
    ...
    din_registry_artifact = HARDHAT_ARTIFACTS / Path("DINModelRegistry.sol/DINModelRegistry.json")
    ...
    task_coord_artifact = HARDHAT_ARTIFACTS / Path("DINTaskCoordinator.sol/DINTaskCoordinator.json")
    ...
    task_auditor_artifact = HARDHAT_ARTIFACTS / Path("DINTaskAuditor.sol/DINTaskAuditor.json")
    ...
    # 3. Copy manifest
    source_manifest = CACHE_DIR / Path(f"{ctx.obj.network.lower()}/model_0/manifest.json")
    ...
    genesis_model_ipfs_hash = get_env_key(ctx.obj.network.upper() + "_" + task_coordinator_address + "_GENESIS_MODEL_IPFS_HASH")
    if not genesis_model_ipfs_hash:
        pytest.fail(f"Could not find {ctx.obj.network.upper()}_{task_coordinator_address}_GENESIS_MODEL_IPFS_HASH in .env")
